...
public class TradeManager extends AbstractVerticle {
    private static final Logger log = LoggerFactory.getLogger(TradeManager.class.getName());

    private final List<JsonObject> transactions = new ArrayList<>();

    private final TradeManagerConfig verticleConfig;
    private KafkaProducer<String, Integer> producer;
    private KafkaConsumer<String, Integer> consumer;

    public TradeManager(TradeManagerConfig verticleConfig) {
        log.info("Creating TradeManager");
    ...
    public void start(Future<Void> start) {
        log.info("Starting TradeManager");
    ...
    private void setupWebServer()   {
        ...
        router.post("/positionmanager").handler(req -> {
            ...
            if (tempAmount instanceof Integer)  {
                ...
                log.error("Failed to convert amount: " + tempAmount);
    ...
    public void stop(Future<Void> stopFuture) {
        log.info("Stopping the TradeManager.");
        producer.close(res -> {
            stopFuture.complete();
        });
    }
